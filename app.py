...
# =========================
# TỌA ĐỘ CẮT KHAY THEO ẢNH GỐC 1920x1080
# =========================
BASE_TRAY_W = 1920
BASE_TRAY_H = 1080
...
def scale_fixed_rois(rois, current_w, current_h):
    """
    Scale tọa độ khay từ ảnh gốc 1920x1080 sang kích thước ảnh hiện tại.
    """
    sx = current_w / BASE_TRAY_W
    sy = current_h / BASE_TRAY_H
...
# =========================
    ...
    st.markdown(
        ...
        <div class="mode-card">
            ...
            <div class="mode-desc">Quý khách chọn đúng mẫu khay. Hệ thống sẽ cắt 5 ô theo tọa độ cố định đã căn chuẩn trên Colab.</div>
        </div>
        """,
        unsafe_allow_html=True
    )

    tray_type_label = st.radio(
        "Chọn loại khay",
        [
            "Loại 1 (3 ô trên + 2 ô dưới)",
            "Loại 2 (2 ô trên + 3 ô dưới)"
    ...
    if tray_type_label == "Loại 1 (3 ô trên + 2 ô dưới)":
        st.session_state.selected_tray_type = "tray1"
    else:
    ...
    if st.session_state.image_rgb is not None and len(st.session_state.detections) > 0:
        ...
        for i, det in enumerate(st.session_state.detections):
            with st.container(border=True):
                ...
                with col_img:
                    st.image(det["crop"], caption=f"Món {i + 1} - {det.get('cell_name', '')}", use_container_width=True)
